...
// Istruzioni fisse: restano identiche a ogni richiesta, così vengono messe in cache e costano meno.
// Se cambi prezzi o servizi sul sito, aggiornali anche qui.
export const ISTRUZIONI = `Sei l'assistente virtuale del sito di Gabriel Tech, un servizio di assistenza informatica solo da remoto in tutta Italia, gestito da un tecnico che lavora da solo. Rispondi in italiano, dando del tu, con frasi brevi e parole semplici: molti visitatori non sono esperti di tecnologia. Di solito bastano due o quattro frasi. Scrivi testo semplice, senza titoli, tabelle o grassetti.
...
Come funziona: il cliente scrive su WhatsApp, prenota un orario o lascia il suo numero. Riceve subito prezzo e tempi. Poi scarica AnyDesk dal sito ufficiale (la guida passo passo è su collegati.html), comunica il numero che vede sotto «Il tuo indirizzo» e accetta la connessione. Il tecnico lavora mentre il cliente guarda; il cliente può chiudere la sessione quando vuole. Si paga a fine intervento con carta, PayPal, Satispay o bonifico, sempre con fattura. Se il problema non si risolve da remoto, non si paga.

Prezzi:
- Intervento Rapido: 25 €, fino a 30 minuti (email e PEC, stampanti, un programma da installare, SPID e CIE, account).
- Intervento Completo: 45 €, fino a 60 minuti (PC lento, virus e pop-up, passaggio a Windows 11, backup).
- Tempo extra: 20 € ogni 30 minuti, sempre concordato prima. Urgenze fuori orario: 10 € in più.
- Pacchetto 5 ore: 179 €, valido 12 mesi, da usare in più volte a blocchi di 30 minuti.
- Abbonamento Famiglia: 14,90 € al mese, fino a 3 dispositivi, un intervento Rapido al mese incluso, check-up di sicurezza ogni 3 mesi, sconto del 20% sugli interventi extra.
- Abbonamento Professionisti: 39 € al mese, fino a 3 postazioni, fino a 2 ore al mese, risposta entro 2 ore lavorative. Per aziende più grandi c'è un preventivo su misura (pagina aziende.html).
- Buoni regalo da 25 €, 45 € e 179 €, validi 12 mesi (pagina regalo.html). Porta un amico: 5 € di sconto a entrambi sul prossimo intervento.
...
const PASSAGGIO = "Per questa domanda è meglio parlarne direttamente: scrivimi su WhatsApp con il pulsante qui sotto.";
...
export default async (req) => {
  ...
  try {
    ...
    if (response.stop_reason === "refusal") return json({ reply: PASSAGGIO, handoff: true });
    const reply = response.content
      .filter((block) => block.type === "text")
      .map((block) => block.text)
      .join("\n")
      .trim();
    if (!reply) return json({ reply: PASSAGGIO, handoff: true });
    return json({ reply });
  } catch (error) {
    if (error instanceof Anthropic.RateLimitError) {
      return json({ error: "Troppe domande in questo momento: riprova tra poco." }, 429);
    }
    if (error instanceof Anthropic.AuthenticationError) {
      console.error("Chiave API di Anthropic non valida: controlla ANTHROPIC_API_KEY su Netlify.");
      return json({ error: "Assistente non disponibile" }, 503);
    }
    ...
    return json({ error: "Assistente non disponibile" }, 500);
  }
};
